...
static qboolean initSurface( void )
{
	...
	for (uint32_t i = 0; i < vk_core.surface.num_surface_formats; ++i)
	{
		...
		gEngine.Con_Reportf("\t%u: %s(%u) %s(%u)\n", i,
			formatName(vk_core.surface.surface_formats[i].format), vk_core.surface.surface_formats[i].format,
			colorspaceName(vk_core.surface.surface_formats[i].colorSpace), vk_core.surface.surface_formats[i].colorSpace);
	}
