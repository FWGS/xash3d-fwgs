...
void CL_BatchResourceRequest( qboolean initialize )
{
	...
	for( p = cl.resourcesneeded.pNext; p && p != &cl.resourcesneeded; p = n )
	{
		...
		case t_sound:
			CL_MoveToOnHandList(p);
			sky::RequireResource("sound/" + std::string(p->szFileName), [p] {
				Q_strncpy(cl.sound_precache[p->nIndex], p->szFileName, sizeof(cl.sound_precache[0]));
				cl.sound_index[p->nIndex] = S_RegisterSound(p->szFileName);
			});
			break;

		case t_model:
			if (p->nIndex == WORLD_INDEX)
				if (!CL_CheckFile(&msg, p))
					break;

			CL_MoveToOnHandList(p);
			sky::RequireResource(p->szFileName, [p] {
				cl.models[p->nIndex] = Mod_ForName(p->szFileName, false, true);
			});
			break;
...
void CL_ParseCustomization( sizebuf_t *msg )
{
	...
	pRes->type = (resourcetype_t)MSG_ReadByte( msg );
...
void CL_ParseResource( sizebuf_t *msg )
{
	...
	pResource->type = (resourcetype_t)MSG_ReadUBitLong( msg, 4 );
