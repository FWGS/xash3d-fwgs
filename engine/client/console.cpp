...
*/
		auto cmd = std::string(con.chat_cmd, std::strlen(con.chat_cmd));
		auto buffer = std::string(con.chat.buffer, std::strlen(con.chat.buffer));
		sky::draw::DrawChatInputField(sky::to_wstring(cmd), sky::to_wstring(buffer), con.chat.cursor);
	}
