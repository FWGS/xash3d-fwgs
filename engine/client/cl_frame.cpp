...
#define FIX_SHITTY_MOUSE_BEHAVIOUR_UNDER_EMSCRIPTEN // savage hack
...
void CL_ExtraUpdate( void )
{
	...
	bool pointer_locked = ev.isActive;

	static auto prev_pointer_locked = pointer_locked;
	static auto framecount_when_first_locked = host.framecount;

	if (prev_pointer_locked != pointer_locked)
	{
		prev_pointer_locked = pointer_locked;

		if (pointer_locked)
			framecount_when_first_locked = host.framecount;
	}

	auto frames_since_locked = host.framecount - framecount_when_first_locked;

	if (pointer_locked)
	{
#ifdef FIX_SHITTY_MOUSE_BEHAVIOUR_UNDER_EMSCRIPTEN
		if (frames_since_locked >= 5)
		{
	...
	{
		Platform_SetCursorType(pointer_locked ? dc_none : dc_arrow);
	}
#else
	clgame.dllFuncs.IN_Accumulate();
#endif
