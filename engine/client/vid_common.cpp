...
engine::glwstate_t	engine::glw_state;
...
void VID_SetDisplayTransform( int *render_w, int *render_h )
{
	...
	if(ref.dllFuncs.R_SetDisplayTransform( (ref_screen_rotation_t)rotate, 0, 0, vid_scale.value, vid_scale.value ))
	{
		if( rotate & 1 )
		{
			int swap = *render_w;
