...
CVAR_DEFINE( engine::cl_lightstyle_lerping, "cl_lightstyle_lerping", "0", FCVAR_ARCHIVE, "enables animated light lerping (perfomance option)" );
...
CVAR_DEFINE( engine::cl_lw, "cl_lw", "1", FCVAR_ARCHIVE|FCVAR_USERINFO, "enable client weapon predicting" );
...
void CL_CheckForResend( void )
{
	...
	if (!NET_IsClientWebsocketConnected())// && cl.maxclients != 1) // TODO: uncomment to fix singleplayer, but it ruin multiplayer
		return;
#endif
...
void CL_Connect_f( void )
{
	...
	NET_Config( true, !cl_nat.value, server ); // allow remote
...
*/
#ifdef XASH_WEBSOCKET
void CL_Disconnect( bool reconnect )
#else
...
#endif
{
	...
	if (!reconnect)
	{
		NET_DestroyClientWebsocket();
	}
#endif
	Mod_FreeAll(); // added by sky
	...
	else if( !Q_strcmp( c, "f" ))
	{
		...
		while( MSG_GetNumBitsLeft( msg ) > 8 )
		{
			...
			if( clgame.request_type == NET_REQUEST_CLIENT && clgame.master_request != NULL )
			{
				...
				list->next = (net_adrlist_t*)nr->resp.response;
...
{
	sky::Emit(sky::InMenuChanged{ true });
	return;
