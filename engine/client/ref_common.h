...
// handy API wrappers
namespace engine { void R_GetTextureParms( int *w, int *h, int texnum ); }
...
#define GL_UpdateTextureInternal( name, pic, flags ) ref.dllFuncs.GL_LoadTextureFromBuffer( (name), (pic), (texFlags_t)(flags), true )
#define R_GetBuiltinTexture( name ) ref.dllFuncs.GL_LoadTexture( (name), 0, 0, 0 )

void GL_RenderFrame( const struct ref_viewpass_s *rvp );

// common engine and renderer cvars
namespace engine { extern convar_t	r_decals; }
extern convar_t	r_adjust_fov;
namespace engine { extern convar_t gl_clear; }

namespace engine { qboolean R_Init( void ); }
namespace engine { void R_Shutdown( void ); }
