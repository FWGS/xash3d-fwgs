...
static void NetGraph_DrawTextFields( int x, int y, int w, wrect_t rect, int count, float avg, int packet_loss, int packet_choke, int graphtype )
{
	...
	if( framerate > 0.0f )
	{
		...
		Q_sprintf( buf, "%.1f fps" , 1.0f / framerate );
		CL_DrawString( x, y, buf, colors, font, FONT_DRAW_NORENDERMODE );

		if( avg > 1.0f )
		{
			Q_sprintf( buf, "%i ms", (int)avg );
		...
		Q_sprintf( buf, "in :  %i %.2f kb/s", netstat_graph[j].msgbytes, cls.netchan.flow[FLOW_INCOMING].avgkbytespersec );
		CL_DrawString( x, y, buf, colors, font, FONT_DRAW_NORENDERMODE );
		y += 15;

		Q_sprintf( buf, "out:  %i %.2f kb/s", out, cls.netchan.flow[FLOW_OUTGOING].avgkbytespersec );
		CL_DrawString( x, y, buf, colors, font, FONT_DRAW_NORENDERMODE );
		y += 15;
		...
		if( graphtype > 2 )
		{
			...
			Q_sprintf( buf, "loss: %i choke: %i", loss, choke );
			CL_DrawString( x, y, buf, colors, font, FONT_DRAW_NORENDERMODE );
		}
	}
	...
	if( graphtype < 3 )
	{
		Q_sprintf( buf, "%i/s", (int)cl_cmdrate->value );
		CL_DrawString( ptx, pty, buf, colors, font, FONT_DRAW_NORENDERMODE );
	}

	Q_sprintf( buf, "%i/s", (int)cl_updaterate->value );
