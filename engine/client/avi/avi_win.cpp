...
//			return length;
	...
	if( Avi->audio_codec == WAVE_FORMAT_PCM )
	{
		...
		pAVIStreamRead( Avi->audio_stream, offset / Avi->audio_bytes_per_sample, length / Avi->audio_bytes_per_sample, audiodata, length, (LONG*)&result, NULL );
		return result;
	}
	else
	{
...
void AVI_OpenVideo( movie_state_t *Avi, const char *filename, qboolean load_audio, int quiet )
{
	...
	while( 1 )
	{
		...
		else if( stream_info.fccType == streamtypeAUDIO && Avi->audio_stream == NULL && load_audio )
		{
			...
			pAVIStreamReadFormat( Avi->audio_stream, pAVIStreamStart( Avi->audio_stream ), Avi->audio_header, (LONG*)&size );
