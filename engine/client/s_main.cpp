...
void S_StreamAviSamples( void *Avi, int entnum, float fvol, float attn, float synctime )
{
	...
	while( ch->s_rawend < soundtime + ch->max_samples )
	{
		...
		// read audio stream
		r = AVI_GetAudioChunk( (movie_state_t*)Avi, (char*)raw, info->loopStart, fileBytes ); // TODO: maybe bug in raw argument, we cast array to char*, maybe we need to do (char*)&raw
