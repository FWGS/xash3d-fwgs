...
static rgbdata_t *ImagePack( void )
{
	...
	pack = (rgbdata_t*)Mem_Calloc( host.imagepool, sizeof( *pack ));
...
rgbdata_t *FS_CopyImage( rgbdata_t *in )
{
	...
	out = (rgbdata_t*)Mem_Malloc( host.imagepool, sizeof( rgbdata_t ));
	*out = *in;
	...
	if( palSize )
	{
		out->palette = (byte*)Mem_Malloc( host.imagepool, palSize );
		memcpy( out->palette, in->palette, palSize );
	}

	if( in->size )
	{
		out->buffer = (byte*)Mem_Malloc( host.imagepool, in->size );
