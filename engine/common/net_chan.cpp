...
void Netchan_OutOfBand( int net_socket, netadr_t adr, int length, byte *data )
{
	...
	{
		// send the datagram
		NET_SendPacket( (netsrc_t)net_socket, MSG_GetNumBytesWritten( &send ), MSG_GetData( &send ), adr );
	}
}
...
void Netchan_UpdateProgress( netchan_t *chan )
{
	...
	for( i = MAX_STREAMS - 1; i >= 0; i-- )
	{
		...
		if( chan->incomingbufs[i] )
		{
			...
			if( total )
			{
				float percent = (float)c / (float)total;
		...
		else if( chan->fragbufs[i] )	// Sending data
		{
			...
			{
				float percent = (float)chan->fragbufs[i]->bufferid / (float)chan->fragbufcount[i];
	...
	scr_download.value = bestpercent * 100.0f;
	sky::OnFileDownloadProgress(host.downloadfile, bestpercent);
