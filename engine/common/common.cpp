...
engine::ConversionPtr engine::Mem_Malloc(poolhandle_t pool, size_t size)
{
	ConversionPtr result;
	result.ptr = _Mem_Alloc(pool, size, false, __FILE__, __LINE__);
	return result;
}

engine::ConversionPtr engine::Mem_Calloc(poolhandle_t pool, size_t size)
{
	ConversionPtr result;
	result.ptr = _Mem_Alloc(pool, size, true, __FILE__, __LINE__);
	return result;
}

engine::ConversionPtr engine::Mem_Realloc(poolhandle_t pool, void* ptr, size_t size)
{
	ConversionPtr result;
	result.ptr = _Mem_Realloc(pool, ptr, size, true, __FILE__, __LINE__);
	return result;
}
...
#ifdef _DEBUG
void engine::DBG_AssertFunction( qboolean fExpr, const char* szExpr, const char* szFile, int szLine, const char* szMessage )
{
	if( fExpr ) return;
...
{
	if (std::string(filename).starts_with("textures/"))
		return true;
...
char *_copystring( poolhandle_t mempool, const char *s, const char *filename, int fileline )
{
	...
	size = Q_strlen( s ) + 1;
	b = (char*)_Mem_Alloc( mempool, size, false, filename, fileline );
	Q_strncpy( b, s, size );
