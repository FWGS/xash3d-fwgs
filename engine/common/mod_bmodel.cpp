...
static void Mod_LoadTextureData( dbspmodel_t *bmod, int textureIndex )
{
	...
	auto tex_path = fmt::format("textures/{}.mip", mipTex->name);

	if (texture->gl_texturenum == 0)
	{
		texture->gl_texturenum = ref.dllFuncs.GL_LoadTexture(tex_path.c_str(), NULL, 0, txFlags);
	}

	// If texture is completely missed:
	if( texture->gl_texturenum == 0 )
	{
		Con_DPrintf( S_WARN "Unable to find %s\n", tex_path.c_str());
		texture->gl_texturenum = R_GetBuiltinTexture(REF_WHITE_TEXTURE);

		// request it from server
		sky::RequireResource(tex_path, [tex_path, texture, txFlags] {
			texture->gl_texturenum = ref.dllFuncs.GL_LoadTexture(tex_path.c_str(), NULL, 0, txFlags);
		});
	}
...
static int Mod_LumpLooksLikeEntities( const char *lump, const size_t lumplen )
{
	...
	return Q_memmem( (const byte*)lump, lumplen, (const byte*)"\"classname\"", sizeof( "\"classname\"" ) - 1 ) != NULL ? 1 : 0;
}
...
qboolean Mod_LoadBmodelLumps( const byte *mod_base, qboolean isworld )
{
	...
	case HLBSP_VERSION:
		...
		else if( !Mod_LumpLooksLikeEntities( (const char*)mod_base + header->lumps[LUMP_ENTITIES].fileofs, header->lumps[LUMP_ENTITIES].filelen ) &&
			 Mod_LumpLooksLikeEntities((const char*)mod_base + header->lumps[LUMP_PLANES].fileofs, header->lumps[LUMP_PLANES].filelen ))
		{
