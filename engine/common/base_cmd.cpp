...
{
	basecmd_test_stats_t *stats = (basecmd_test_stats_t*)ptr;
...
void BaseCmd_Test_f( void )
{
	...
	for( i = 0; i < 1000; i++ )
	{
		...
		for( cmd = Cmd_GetFirstFunctionHandle(); cmd;
			 cmd = Cmd_GetNextFunctionHandle( (struct cmd_s*)cmd ), stats.lookups++ )
		{
			if( !BaseCmd_Find( HM_CMD, Cmd_GetName((struct cmd_s*)cmd )))
			{
				Con_Printf( "Command %s is missing in basecmd\n", Cmd_GetName((struct cmd_s*)cmd ));
				stats.valid = false;
			}
		}
