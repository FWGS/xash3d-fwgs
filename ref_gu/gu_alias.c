/*
gu_alias.c - alias model renderer
...
void GL_DrawAliasFrame( aliashdr_t *paliashdr )
{
	...
	while( 1 )
	{
		...
		{
			out[index].u = ((float *)order)[0];
			out[index].v = ((float *)order)[1];
			order += 2;
			...
			out[index].c = GUCOLOR4F( g_alias.lightcolor[0] * lv_tmp, g_alias.lightcolor[1] * lv_tmp, g_alias.lightcolor[2] * lv_tmp, tr.blend );
...
static void R_AliasDrawAbsBBox( cl_entity_t *e, const vec3_t absmin, const vec3_t absmax )
{
	...
	sceGuColor( GUCOLOR4F( 0.5f, 0.5f, 1.0f, 0.5f ) );
	TriRenderMode( kRenderTransAdd );
	sceGuTexFunc( GU_TFX_MODULATE, GU_TCC_RGBA );

	unsigned int color = getTriBrightness( g_alias.shadelight / 255.0f );

	gu_vert_fcv_t* const out = ( gu_vert_fcv_t* )sceGuGetMemory( sizeof( gu_vert_fcv_t ) * 24 );
	for( i = 0; i < 6; i++ )
	{
		out[i * 4 + 0].c = color;
		out[i * 4 + 0].x = p[boxpnt[i][0]][0];
		out[i * 4 + 0].y = p[boxpnt[i][0]][1];
		out[i * 4 + 0].z = p[boxpnt[i][0]][2];
		out[i * 4 + 1].c = color;
		out[i * 4 + 1].x = p[boxpnt[i][1]][0];
		out[i * 4 + 1].y = p[boxpnt[i][1]][1];
		out[i * 4 + 1].z = p[boxpnt[i][1]][2];
		out[i * 4 + 2].c = color;
		out[i * 4 + 2].x = p[boxpnt[i][2]][0];
		out[i * 4 + 2].y = p[boxpnt[i][2]][1];
		out[i * 4 + 2].z = p[boxpnt[i][2]][2];
		out[i * 4 + 3].c = color;
		out[i * 4 + 3].x = p[boxpnt[i][3]][0];
		out[i * 4 + 3].y = p[boxpnt[i][3]][1];
		out[i * 4 + 3].z = p[boxpnt[i][3]][2];
	}
	sceGuDrawArray( GU_TRIANGLE_FAN, GU_TEXTURE_32BITF | GU_VERTEX_32BITF, 24, 0, out );
...
static void R_AliasDrawLightTrace( cl_entity_t *e )
{
	if( r_drawentities->value == 7 )
	{
		...
		gu_vert_fcv_t* out = ( gu_vert_fcv_t* )sceGuGetMemory( sizeof( gu_vert_fcv_t ) * 4 );
		out[0].c = GU_COLOR( 1.0f, 0.5f, 0.0f, 1.0f );
		out[0].x = e->origin[0];
		out[0].y = e->origin[1];
		out[0].z = e->origin[2];
		out[1].c = out[0].c;
		out[1].x = g_alias.lightspot[0];
		out[1].y = g_alias.lightspot[1];
		out[1].z = g_alias.lightspot[2];
		VectorMA( g_alias.lightspot, -64.0f, g_alias.lightvec, origin );
		out[2].c = GU_COLOR( 0.0f, 0.5f, 1.0f, 1.0f );
		out[2].x = g_alias.lightspot[0];
		out[2].y = g_alias.lightspot[1];
		out[2].z = g_alias.lightspot[2];
		out[3].c = out[2].c;
		out[3].x = origin[0];
		out[3].y = origin[1];
		out[3].z = origin[2];
		sceGuDrawArray( GU_LINES, GU_COLOR_8888 | GU_VERTEX_32BITF, 4, 0, out );

		out = ( gu_vert_fcv_t* )sceGuGetMemory( sizeof( gu_vert_fcv_t ) );
		out->c = GU_COLOR( 1.0f, 0.0f, 0.0f, 1.0f );
		out->x = g_alias.lightspot[0];
		out->y = g_alias.lightspot[1];
		out->z = g_alias.lightspot[2];
		sceGuDrawArray( GU_POINTS, GU_COLOR_8888 | GU_VERTEX_32BITF, 1, 0, out );
...
void R_DrawAliasModel( cl_entity_t *e )
{
	...
	if( r_shadows.value )
	{
		...
		sceGuColor( GUCOLOR4F( 0.0f, 0.0f, 0.0f, 0.5f ) );
