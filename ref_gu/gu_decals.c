...
void DrawSingleDecal( decal_t *pDecal, msurface_t *fa )
{
	...
	gu_vert_t* const out = ( gu_vert_t* )sceGuGetMemory( sizeof( gu_vert_t ) * numVerts );
	for( i = 0; i < numVerts; i++, v += VERTEXSIZE )
	{
		out[i].uv[0] = v[3];
		out[i].uv[1] = v[4];
		VectorCopy( v, out[i].xyz );
	}
...
void DrawSurfaceDecals( msurface_t *fa, qboolean single, qboolean reverse )
{
	...
	if( single )
	{
		...
		if( gl_depthoffset->value )
			sceGuDepthOffset( ( int )-gl_depthoffset->value );
	}
	...
	if( FBitSet( fa->flags, SURF_TRANSPARENT ) && glState.stencilEnabled )
	{
		...
		for( p = fa->pdecals; p; p = p->pnext )
		{
			if( p->texture )
			{
				...
				{
					out[i].uv[0] = ( DotProduct( v, tex->vecs[0] ) + tex->vecs[0][3] ) / tex->texture->width;
					out[i].uv[1] = ( DotProduct( v, tex->vecs[1] ) + tex->vecs[1][3] ) / tex->texture->height;
					VectorCopy( v, out[i].xyz );
				}
				sceGuDrawArray( GU_TRIANGLE_FAN, GU_TEXTURE_32BITF | GU_VERTEX_32BITF, numVerts, 0, out );
#if 0 /* two layer ??? */
